...
for i = 1:numStimuli %add instances of boith stimuli being the same
    combinations(length(combinations)+ 1, :) = i;
end
...
for trialNum = 1:length(lateralisedRecord)%coding the lighting
    for face = 1:2:3
        ...
        elseif lateralisedRecord(trialNum,face) >10 & lateralisedRecord(trialNum,face) <= 15
            MLCMFood(trialNum, face+1) = 3;
        elseif lateralisedRecord(trialNum,face) >15 & lateralisedRecord(trialNum,face) <= 20
            MLCMFood(trialNum, face+1) = 4;
        else 
            MLCMFood(trialNum, face+1) = 5;
        end 
    end
end
